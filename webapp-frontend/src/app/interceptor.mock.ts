...
export class HttpMockRequestInterceptor implements HttpInterceptor {
  ...
  intercept(
    ...
    if (request.method === "PUT" && request.url.includes("policies")) {
      this.result = of(new HttpResponse({ status: 200 }));
    }
    if (
      request.url === "/a1-policy/v2/policy-types" &&
    ...
    for (const element of urls) {
      if (request.url === element.url) {
        if (request.method === "DELETE") {
          this.result = of(new HttpResponse({ status: 204 }));
        } else {
          this.result = of(
            new HttpResponse({
              status: 200,
    ...
    if (this.result) {
      console.log(
        "Loaded from mock http call :" + request.method + " " + request.url, (request.method === "PUT" ? request.body : "")
      );
      // Adding a delay to simulate real server call.
      return this.result.pipe(delay(10));
    } else {
