...
.. note::
   It is important to note that all the route configurations are provided in application.yaml of the gateway,
   so in case domain name and port for Policy Management Service and Enrichment Information Coordinator Service
   are not the default ones, application.yaml file must be modified.
